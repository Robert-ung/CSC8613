...
def upsert_csv(table: str, csv_path: str, pk_cols: list[str]):
    ...
    # Conversion en booléen des colonnes si elles existent
    ...
    with eng.begin() as conn:
        ...
        # Construction de la partie SET col = EXCLUDED.col pour toutes les colonnes non PK
        updates = ", ".join(
...
def validate_with_ge(table: str):
    ...
    if table == "users":
        ...
        # Vérifier que les colonnes correspondent exactement à l'ensemble attendu
        gdf.expect_table_columns_to_match_set([
        "user_id",
        "watch_hours_30d",
        "avg_session_mins_7d",
        ...
        # Vérifier que l'identifiant utilisateur n'est pas nul
        gdf.expect_column_values_to_not_be_null("user_id")

        # Vérifier que certaines colonnes numériques sont bien non négatives
